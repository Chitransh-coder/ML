...
`save data.mat` Save your current workspace to a MAT-file.
`load data.mat` Load the variables in a MAT-file to the workspace.
`clear` Clear all variables from the workspace.
`clc` Clear all text from the Command Window.
`format long` Change how numeric output appears in the Command Window.
...
**Array Operations** - Multiple operations can be performed with arrays.
...
`(o)` circle marker, with a heavy line width.
...
`xlabel("x") # or ylabel("y")` - Add labels to axes.
...
**Programming** - ***MATLAB*** supports *if...else* statements and *for* loop.
...
>If x is greater than 0.5, set y to 3. Otherwise, set y to 4.
